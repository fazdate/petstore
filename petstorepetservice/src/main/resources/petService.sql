...
(
    ...
    pet_name              varchar(64)   not null unique,
    category_id       bigserial     not null,
    pet_photoURL          varchar(255)  not null,
...
(
    pet_id            bigserial     not null references pet (pet_id) on delete cascade,
    tag_id            bigserial     not null references tag (tag_id) on delete cascade,
    constraint pet_tag_pkey primary key (pet_id, tag_id)
);

insert into petstorepetservice_db.public.category (category_id, category_name)
values (1, 'Dog'),
       (2, 'Cat'),
       (3, 'Fish')
ON CONFLICT (category_id) DO NOTHING;

insert into petstorepetservice_db.public.tag (tag_id, tag_name)
values (1, 'doggie'),
       (2, 'large'),
       (3, 'small'),
       (4, 'kittie'),
...
insert into petstorepetservice_db.public.pet (pet_id, pet_name, category_id, pet_photoURL, status)
